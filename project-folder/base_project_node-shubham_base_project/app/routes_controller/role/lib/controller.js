...
const { common, dbCommon, enums, messages } = require("../../../../utils");
  ...
  try {
    ...
    if (response && response.status != status.OK) {
      return res.status(status.BadRequest).json({
        message: messages.ROLE_ALREADY_EXISTS || messages.ALREADY_EXISTS,
...
exports.findAll = async (req, res) => {
  try {
    ...
    if (req.path.endsWith("options")) {
      whereCondition.status = enums.Status.Active;
      options.attributes = [
  ...
  try {
    ...
    if (role.systemDefault) {
      return res
        ...
        .json({ message: "Can not de-activate System Default Role." });
    }

    role.set({
      status:
        role.status === enums.Status.Active
  ...
  try {
    ...
    if (role.systemDefault) {
      return res
        ...
        .json({ message: "System Default Role can not be deleted." });
    }
    ...
    if (userCount > 0) {
      return res
        .status(status.BadRequest)
        .json({ message: "Users are associate with this Role!" });
    }
  ...
  try {
    ...
    if (tempCount != req.body.roleIds.length) {
      ...
      return res
        ...
        .json({ message: "Improper data present." });
    }
    ...
    await dbCommon.myBulkUpdate(priorityData, db.Role, "id", transaction);
