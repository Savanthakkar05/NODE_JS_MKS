...
const managedByFullName = db.Sequelize.fn(
  "concat",
  db.Sequelize.col("ManagedByUser.firstName"),
  " ",
  db.Sequelize.col("ManagedByUser.lastName")
);

const approvalByFullName = db.Sequelize.fn(
  "concat",
  db.Sequelize.col("ApprovalUser.firstName"),
  " ",
  db.Sequelize.col("ApprovalUser.lastName")
);
const instituteUserFullName = db.Sequelize.fn(
  "concat",
  db.Sequelize.col("Institute.User.firstName"),
  " ",
  db.Sequelize.col("Institute.User.lastName")
);
...
const pagination = (body) => {
  ...
  // Take Skip from body.
  if (body?.skip) {
    skipAndTake.offset = Number(body.skip);
  }
  // Take Limit from body.
  if (body?.take) {
    skipAndTake.limit = Number(body.take);
  }
...
const sortWithLiteral = (item, str, model, colOpt = null) => {
  ...
  if (!isFK) {
    ...
    if (item.groupInterval)
      selector = `${item.groupInterval}([${model}].[${str}])`;
  } else {
    if (item.groupInterval) selector = `${item.groupInterval}(${str})`;
  }

  return [
    db.Sequelize.literal(`${selector}`),
    item?.desc == true ? "DESC" : "ASC",
  ];
};
 ...
 */
const generateSortable = async (body, model, columnOptions) => {
  let modelSortable = [];

  // fields to be sorted with. Default [].
  let mainSort = body.sort ? body.sort : [];

  // fields to be sorted when group is used. Default [].
  let groupSort = body?.group ? body?.group : [];
    ...
    if (!isFK) {
      attributes = [
        [
          db.Sequelize.fn(
            "DISTINCT",
            db.Sequelize.col(`[${model}].[${groupData[0].selector}]`)
          ),
          "key",
        ],
        [db.Sequelize.fn("COUNT", db.Sequelize.literal("*")), "count"],
    ...
    // when selector is foreign key.
      ...
      if (IsMultilevelFk) {
        ...
        attributes = [
          ...
          [
            db.Sequelize.fn("COUNT", db.Sequelize.col(groupData[0].selector)),
            "count",
          ],
        ];
      } else {
...
const formatField = (fieldName, operator, fieldValue, columnOptions, model) => {
  // Replace $*$ with %. As junk value is generated from req.body.
    ...
    obj[Op.and].push(
      db.Sequelize.where(columnOpt.value, {
        [sequelizeOperators[operator].key]:
          sequelizeOperators[operator].starts +
          `${fieldValue}` +
          sequelizeOperators[operator].ends,
      })
    );
  } else {
    obj[fieldKey] = {
...
/**
 *
 * @param {*} queryFilters filters from body.
 * @param {*} model model name
 * @param {*} columnOptions column options
...
const generateFilters = (queryFilters, model, columnOptions) => {
  ...
  if (queryFilters[0] == "!") isNotOperator = true;
    ...
    .map((condition) => {
      ...
      if (Array.isArray(condition)) {
        ...
        if (condition.find((f1) => Array.isArray(f1))) {
          if (isNotOperator) {
            return {
              [Op.not]: generateFilters(condition, model, columnOptions),
            };
          } else {
 ...
 * @param {*} queryGroup group from body.
...
/**
 *
 * @param {*} totalSummary summary object from body
 * @param {*} model model name
  ...
  if (totalSummary.length) {
    totalSummary
      ...
      .map((item, index) => {
        ...
        if (colOpt && colOpt.isCustom) {
          summaryAttributes.push([
            db.Sequelize.fn(summaryType, colOpt.value),
            `${summaryType} + ${index}`,
          ]);
        } else {
          summaryAttributes.push([
            db.Sequelize.fn(summaryType, db.Sequelize.col(fieldName)),
            `${summaryType} + ${index}`,
          ]);
        }
      });
...
/**
 *
 * @param {*} queryGroup group data from body.
...
const formattingGroupedData = async (queryGroup, allData) => {
  ...
  if (!queryGroup.length > 1) {
    ...
    if (
      queryGroup.length > 0 &&
      queryGroup.find(
        (f1) => f1.groupInterval && dateGroup.includes(f1.groupInterval)
      )
    ) {
...
/**
 *
 * @param {*} body req.body which contains DevExtreme filter object.
 ...
 */
const devExtremeFilters = async (body, model, includes, columnOptions = {}) => {
  // adding default column options for createdBy and updatedBy
  columnOptions["CreatedByUser.fullName"] = {
    isCustom: true,
    value: createdByFullName,
    isDefault: true,
  };
  columnOptions["UpdatedByUser.fullName"] = {
    isCustom: true,
    value: updatedByFullName,
    isDefault: true,
  };
  columnOptions["ManagedByUser.fullName"] = {
    isCustom: true,
    value: managedByFullName,
    isDefault: true,
  };
  columnOptions["ApprovalUser.fullName"] = {
    isCustom: true,
    value: approvalByFullName,
    isDefault: true,
  };
  columnOptions["Institute.User.fullName"] = {
    isCustom: true,
    value: instituteUserFullName,
    isDefault: true,
  };
  ...
  //* pagination
  if (body?.skip || body?.take) skipAndTake = pagination(body);

  //* Sorting is applied when Sort object is present in body as well as when grouping.
  if (body?.sort || body?.group)
    sortOrder = await generateSortable(body, model, columnOptions);

  //* Summary
  if (body?.totalSummary) {
    let result = generateSummaries(body.totalSummary, model, columnOptions);
  ...
  // ** search filters
  if (body?.filter) {
    let queryFilters = body.filter ? body.filter : [];
  ...
  if (Object.keys(columnOptions).length > 0 && !body.group) {
    let addCustomCols = [];
  ...
  if (body.group) {
    if (attributes && Array.isArray(attributes)) {
      attributes.push([
        db.Sequelize.fn("MAX", db.Sequelize.col(`${model}.id`)),
        "id",
      ]);
    }
